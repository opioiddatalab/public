...
(function( root, factory ) {
	...
	var SLIDES_SELECTOR = '.slides section',
		...
		FRAGMENT_STYLE_REGEX = /fade-(down|up|right|left|out|in-then-out|in-then-semi-out)|semi-fade-out|current-visible|shrink|grow/,
	...
	function slide( h, v, f, o ) {
		...
		var autoAnimateTransition = false;
		...
		if( currentSlide ) dom.statusDiv.textContent = getStatusText( currentSlide );
	...
	function autoAnimate( fromSlide, toSlide ) {
		...
		if( fromSlide.hasAttribute( 'data-auto-animate' ) && toSlide.hasAttribute( 'data-auto-animate' ) ) {
			...
			var css = getAutoAnimatableElements( fromSlide, toSlide ).map( function( elements ) {
				return autoAnimateElements( elements.from, elements.to, elements.options || {}, animationOptions, autoAnimateCounter++ );
			} );
			...
			if( toSlide.dataset.autoAnimateUnmatched !== 'false' && config.autoAnimateUnmatched === true ) {
				...
				var defaultUnmatchedDuration = animationOptions.duration * 0.8,
					defaultUnmatchedDelay = animationOptions.duration * 0.2;

				getUnmatchedAutoAnimateElements( toSlide ).forEach( function( unmatchedElement ) {

					var unmatchedOptions = getAutoAnimateOptions( unmatchedElement, animationOptions );
					var id = 'unmatched';
	 ...
	 */
	function autoAnimateElements( from, to, elementOptions, animationOptions, id ) {
		...
		if( to.classList.contains( 'fragment' ) ) {
			...
			if( from.classList.contains( 'fragment' ) ) {
				...
				var fromFragmentStyle = ( from.className.match( FRAGMENT_STYLE_REGEX ) || [''] )[0];
				var toFragmentStyle = ( to.className.match( FRAGMENT_STYLE_REGEX ) || [''] )[0];
	...
	function availableFragments() {
		...
		if( currentSlide && config.fragments ) {
			var fragments = currentSlide.querySelectorAll( '.fragment:not(.disabled)' );
			var hiddenFragments = currentSlide.querySelectorAll( '.fragment:not(.disabled):not(.visible)' );
	...
	function navigateFragment( index, offset ) {
		...
		if( currentSlide && config.fragments ) {
			...
			var fragments = sortFragments( currentSlide.querySelectorAll( '.fragment:not(.disabled)' ) );
			if( fragments.length ) {

				// If no index is specified, find the current
				if( typeof index !== 'number' ) {
					var lastVisibleFragment = sortFragments( currentSlide.querySelectorAll( '.fragment:not(.disabled).visible' ) ).pop();
